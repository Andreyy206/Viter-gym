...
const Accesories = () => {
  // const getAllAccesories = result?.data.length > 0 && result?.data.filter((item) => items.items_accesories === "Accesories")
  ...
  return (
    <section className='overflow-x-hidden '>
      <h1 className='font-bold uppercase pb-6 px-5'>Accesories</h1>
        ...
        {!isLoading &&
          result?.data.map((item, key) => {
            return (
              ...
              >
                <img
                  src={`${imgPath}/${item.accesories_photo}`}
                  alt=''
                  className='w-full h-[450px] object-cover'
                />
                <div className='bg-dark p-8'>
                  <h2 className='text-white '>{item.accesories_title}</h2>
                  <p className='text-white '>{item.accesories_details}</p>
                  <div className='grid grid-cols-[1fr,_100px] text-white mx-auto items-center'>
                    <h2 className='text-white'>{item.accesories_price}</h2>
